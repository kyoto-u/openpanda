...
def load_properties(fh, mapping=dict, keep_comments=False):
  """
  ...
  """
  return mapping(iter_properties(fh,keep_comments))
...
def iter_properties(fh, keep_comments):
  """
  ...
  if keep_comments:
    count = 0
    for line in _property_lines_with_comments(fh):
      if len( line ) == 0:
        key = '#@' + str( count )
        value = ''
        count = count + 1
...
def _property_lines(fp):
  ...
  for line in _universal_newlines(fp):
    ...
    if continuation and len(body) + len(backslashes) == 0:
      yield bytes(buf)
      buf = bytearray()
      continue
...
def _property_lines_with_comments(fp):
  buf = bytearray()
  continuation = False
  for line in _universal_newlines(fp):
    m = _LINE_PATTERN.match(line)
