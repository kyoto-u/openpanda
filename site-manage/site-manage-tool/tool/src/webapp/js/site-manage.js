...
var setupCategTools = function(){
   ...
   var showAlert = function(e){
        ...
        $(e.target).parent().parent('li').append('<div id=\"alertBox\">' + msg + '<a href=\"#\" id=\"alertBoxYes\">' + yesMsg + '</a>&nbsp;|&nbsp;<a href=\"#\" id=\"alertBoxNo\">' + noMsg + '</a></div>');
        $(e.target).parent().find('#alertBox').css({
            'top': pos.top + 14,
            'left': pos.left - 150
        });
        $('#alertBox a#alertBoxYes').on('click', function(){
            $(this).parent('div').prev('span').children('a').removeClass('toolInstance').click();
            $('#alertBox').remove();
        });
